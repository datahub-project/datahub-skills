...
Run a data incident end to end: recall prior post-mortems, walk column-level lineage upstream until a node fails on its own rather than inheriting a parent's failure, rank the downstream blast radius by usage, resolve owners, and store a structured post-mortem back in DataHub so the next incident on the same path is cheaper. Writes require explicit approval.

```
> Why is daily_revenue failing?
> What is the blast radius of raw_events?
> /datahub-incident-triage triage the freshness breach on customer_orders
```
...
> /datahub-incident-triage why is daily_revenue failing?
...
| `/catalog-incident-triage [entity]` | Triage an incident to root cause, blast radius, and post-mortem |
...
│   │   ├── references/
│   │   └── templates/
│   ├── datahub-incident-triage/     # End-to-end incident triage
│   │   └── SKILL.md
