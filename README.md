...
Take a symptom — a number that moved, a column that went null — and drive it to an evidence-backed root cause. Traverses upstream and column-level lineage to localize the fault, eliminates competing hypotheses on magnitude and onset, scopes a remediation, verifies it, and resolves the incident in DataHub.

```
> Revenue on the exec dashboard is 40x too high since last Tuesday — why?
> Investigate the active incident on the orders table
> /catalog-investigate root cause the null spike in customer_email
```
...
> /catalog-investigate why is daily revenue 40x too high since Tuesday?
...
cp -r datahub-skills/skills/datahub-incident-investigation your-project/.agents/skills/
...
| `/catalog-investigate [symptom]` | Root cause a data incident and resolve it       |
...
│   │   ├── references/
│   │   └── templates/
│   ├── datahub-incident-investigation/  # Incident root cause analysis
│   │   ├── SKILL.md
│   │   ├── references/
│   │   └── templates/
...
- Catalog interaction skills: `skills/datahub-search/`, `skills/datahub-enrich/`, `skills/datahub-lineage/`, `skills/datahub-quality/`, `skills/datahub-incident-investigation/`, `skills/datahub-setup/`
