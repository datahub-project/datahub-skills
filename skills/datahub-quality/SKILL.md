...
| Diagnose a live incident — blast radius, root cause, postmortem | `/datahub-incident-triage` |
...
- "Raise an incident on orders" → **Quality** (incident record management)
- "orders has been null since 03:00, what broke and who do I tell?" → **Incident triage** (diagnosis)

Quality **records and manages** incidents; incident triage **investigates** one and writes the diagnosis back. They compose — a triage can end by raising a formal incident here.
